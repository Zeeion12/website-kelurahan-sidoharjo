...
import {
  ...
  initialOrtu,
  initialSaksi,
  initialDataPerkawinan,
  type Pelapor,
  type Subyek,
  type OrangTuaRingkas,
  type DataPerkawinan,
  type Saksi,
...
const initialDataPembatalan: DataPembatalan = {
  tanggalPerkawinan: "",
  nomorAktaPerkawinan: "",
...
type FormState = {
  ...
  suami: Subyek;
  istri: Subyek;
  ortuSuami: OrangTuaRingkas;
  ortuIstri: OrangTuaRingkas;
...
const initialState: FormState = {
  ...
  suami: { ...initialSubyek },
  istri: { ...initialSubyek },
  ortuSuami: { ...initialOrtu },
  ortuIstri: { ...initialOrtu },
  perkawinan: { ...initialDataPerkawinan },
  pembatalan: { ...initialDataPembatalan },
  saksi1: { ...initialSaksi },
  saksi2: { ...initialSaksi },
};

export default function FormPembatalanPerkawinanPage() {
  ...
  function validate() {
    const required: [keyof FormState, string, string][] = [
      ...
      ["istri", "nik", "NIK istri"],
      ["perkawinan", "statusSebelumKawin", "Status perkawinan sebelum kawin"],
      ["perkawinan", "tanggalPembatalanPerkawinan", "Tanggal pembatalan perkawinan"],
      ["perkawinan", "tanggalMelapor", "Tanggal melapor"],
      ["perkawinan", "jamPelaporan", "Jam pelaporan"],
      ["perkawinan", "agama", "Agama"],
      ["pembatalan", "tanggalPerkawinan", "Tanggal perkawinan"],
      ["pembatalan", "nomorAktaPerkawinan", "Nomor akta perkawinan"],
      ["pembatalan", "tanggalAktaPerkawinan", "Tanggal akta perkawinan"],
    ...
    <main className="min-h-screen bg-[#FAF8F3] text-[#1E2A4A]">
      <header className="border-b border-[#1E2A4A]/10 bg-white/60">
        <div className="mx-auto max-w-3xl px-6 py-10 md:py-14">
          ...
          <h1 className={`${lora.className} mt-4 text-3xl font-semibold leading-tight md:text-4xl`}>
            Formulir Pembatalan Perkawinan
          </h1>
          <p className="mt-3 max-w-2xl text-sm text-[#1E2A4A]/70 md:text-base">
            Lengkapi data pelapor, data suami-istri, data perkawinan yang
      ...
      <div className="mx-auto max-w-3xl px-6 py-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Section title="Data Orang Tua Suami">
            <OrtuRingkasFields
              data={form.ortuSuami}
              onChange={(field, value) => update("ortuSuami", field, value)}
            />
          </Section>

          <Section title="Data Orang Tua Istri">
            <OrtuRingkasFields
              data={form.ortuIstri}
              onChange={(field, value) => update("ortuIstri", field, value)}
            />
          </Section>

          <Section title="Data Perkawinan">
            <PerkawinanFields
              data={form.perkawinan}
              onChange={(field, value) => update("perkawinan", field, value)}
              errors={errors}
          ...
          <Section
            title="Data Pembatalan Perkawinan"
            hint="Data perkawinan yang diajukan untuk dibatalkan"
          >
            <Field
              label="Tanggal Perkawinan"
              required
              error={errors["pembatalan.tanggalPerkawinan"]}
            >
              <input
                type="date"
                className={
                  errors["pembatalan.tanggalPerkawinan"] ? errorInputClass : inputClass
                }
                value={form.pembatalan.tanggalPerkawinan}
                onChange={(e) => update("pembatalan", "tanggalPerkawinan", e.target.value)}
              />
            </Field>
            <Field
              label="Nomor Akta Perkawinan"
              required
              error={errors["pembatalan.nomorAktaPerkawinan"]}
            >
              <input
                className={
                  errors["pembatalan.nomorAktaPerkawinan"] ? errorInputClass : inputClass
                }
                value={form.pembatalan.nomorAktaPerkawinan}
                onChange={(e) =>
                  update("pembatalan", "nomorAktaPerkawinan", e.target.value)
                }
              />
            </Field>
            <Field
              ...
              required
              error={errors["pembatalan.tanggalAktaPerkawinan"]}
            >
              <input
                type="date"
                className={
                  errors["pembatalan.tanggalAktaPerkawinan"] ? errorInputClass : inputClass
                }
                value={form.pembatalan.tanggalAktaPerkawinan}
                onChange={(e) =>
                  update("pembatalan", "tanggalAktaPerkawinan", e.target.value)
                }
              />
            </Field>
            <Field label="Nama Pengadilan">
              <input
                className={inputClass}
                value={form.pembatalan.namaPengadilan}
                onChange={(e) => update("pembatalan", "namaPengadilan", e.target.value)}
              />
            </Field>
            <Field label="Nomor Putusan Pengadilan">
              <input
                className={inputClass}
            ...
            <Field label="Tanggal Putusan Pengadilan">
              <input
                type="date"
                className={inputClass}
            ...
            <Field label="Tanggal Pelaporan Perkawinan" hint="Khusus perkawinan di luar negeri">
              <input
                type="date"
                className={inputClass}
