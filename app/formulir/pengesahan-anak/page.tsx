...
type DataAktaAnak = {
  ...
  tanggalKelahiranAnak: string;
  tanggalPerkawinanAgama: string;
};

type DataAktaPerkawinanOrtu = {
...
type FormState = {
  pelapor: Pelapor;
  akta: DataAktaAnak;
  aktaPerkawinanOrtu: DataAktaPerkawinanOrtu;
...
const initialState: FormState = {
  ...
  akta: {
    ...
    tanggalKelahiranAnak: "",
    tanggalPerkawinanAgama: "",
  },
  aktaPerkawinanOrtu: {
    nomorAktaPerkawinan: "",
...
export default function FormPengesahanAnakPage() {
  ...
  function validate() {
    const required: [keyof FormState, string, string][] = [
      ...
      ["akta", "nomorAktaKelahiran", "Nomor akta kelahiran"],
      ["akta", "tanggalKelahiranAnak", "Tanggal kelahiran anak"],
      ["aktaPerkawinanOrtu", "nomorAktaPerkawinan", "Nomor akta perkawinan/buku nikah"],
      ["aktaPerkawinanOrtu", "tanggalAktaPerkawinan", "Tanggal akta perkawinan/buku nikah"],
    ...
    <main className="min-h-screen bg-[#FAF8F3] text-[#1E2A4A]">
      <header className="border-b border-[#1E2A4A]/10 bg-white/60">
        <div className="mx-auto max-w-3xl px-6 py-10 md:py-14">
          ...
          <h1 className={`${lora.className} mt-4 text-3xl font-semibold leading-tight md:text-4xl`}>
            Formulir Pencatatan Pengesahan Anak
          </h1>
          <p className="mt-3 max-w-2xl text-sm text-[#1E2A4A]/70 md:text-base">
            Lengkapi data pelapor, data akta kelahiran anak, data akta
            perkawinan orang tua, data orang tua kandung, data penetapan
            pengadilan (bila ada), serta data saksi sesuai dokumen yang
            Bapak/Ibu miliki.
          </p>
      ...
      <div className="mx-auto max-w-3xl px-6 py-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Section title="Data Akta Kelahiran Anak">
            <Field
              label="Nomor Akta Kelahiran"
              required
              error={errors["akta.nomorAktaKelahiran"]}
            >
              <input
                className={errors["akta.nomorAktaKelahiran"] ? errorInputClass : inputClass}
                value={form.akta.nomorAktaKelahiran}
                onChange={(e) => update("akta", "nomorAktaKelahiran", e.target.value)}
              />
            </Field>
            <Field label="Tanggal Penerbitan Akta Kelahiran">
              <input
                ...
                value={form.akta.tanggalPenerbitanAktaKelahiran}
                onChange={(e) =>
                  update("akta", "tanggalPenerbitanAktaKelahiran", e.target.value)
                }
              />
            </Field>
            <Field label="Dinas Kabupaten/Kota yang Menerbitkan">
              <input
                className={inputClass}
                value={form.akta.dinasPenerbitAktaKelahiran}
                onChange={(e) =>
                  update("akta", "dinasPenerbitAktaKelahiran", e.target.value)
                }
              />
            </Field>
            <Field
              label="Tanggal Kelahiran Anak"
              required
              error={errors["akta.tanggalKelahiranAnak"]}
            >
              <input
                type="date"
                className={
                  errors["akta.tanggalKelahiranAnak"] ? errorInputClass : inputClass
                }
                value={form.akta.tanggalKelahiranAnak}
                onChange={(e) => update("akta", "tanggalKelahiranAnak", e.target.value)}
              />
            </Field>
            <Field
              label="Tanggal Perkawinan Menurut Agama"
              hint="Tanggal orang tua menikah secara agama"
            >
              <input
                type="date"
                className={inputClass}
                value={form.akta.tanggalPerkawinanAgama}
                onChange={(e) => update("akta", "tanggalPerkawinanAgama", e.target.value)}
              />
            </Field>
          </Section>

          <Section
            title="Data Akta Perkawinan Orang Tua"
            hint="Akta Perkawinan atau Buku Nikah"
          >
            <Field
              label="Nomor Akta Perkawinan/Buku Nikah"
              required
              error={errors["aktaPerkawinanOrtu.nomorAktaPerkawinan"]}
            >
              <input
                className={
                  errors["aktaPerkawinanOrtu.nomorAktaPerkawinan"]
                    ? errorInputClass
                    : inputClass
                }
                value={form.aktaPerkawinanOrtu.nomorAktaPerkawinan}
                onChange={(e) =>
                  update("aktaPerkawinanOrtu", "nomorAktaPerkawinan", e.target.value)
                }
              />
            </Field>
            <Field
              label="Tanggal Akta Perkawinan/Buku Nikah"
              required
              error={errors["aktaPerkawinanOrtu.tanggalAktaPerkawinan"]}
            >
              <input
                type="date"
                className={
                  errors["aktaPerkawinanOrtu.tanggalAktaPerkawinan"]
                    ? errorInputClass
                    : inputClass
                }
                value={form.aktaPerkawinanOrtu.tanggalAktaPerkawinan}
                onChange={(e) =>
                  update("aktaPerkawinanOrtu", "tanggalAktaPerkawinan", e.target.value)
                }
              />
            </Field>
          </Section>
          ...
          <Section
            title="Data Penetapan Pengadilan"
            hint="Isi bila pengesahan anak melalui penetapan pengadilan"
          >
            ...
            <Field label="Nama Lembaga Pengadilan">
              <input
                className={inputClass}
                value={form.pengadilan.namaLembagaPengadilan}
                onChange={(e) =>
                  update("pengadilan", "namaLembagaPengadilan", e.target.value)
                }
              />
            </Field>
          </Section>
